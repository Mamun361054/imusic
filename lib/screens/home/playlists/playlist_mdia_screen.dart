...
class PlaylistMediaScreen extends StatelessWidget {
  ...
  const PlaylistMediaScreen(
      {Key? key, this.playlists, this.searchPlayList, this.fromLocal = true})
      : super(key: key);
    ...
    return Scaffold(
      ...
      body: fromLocal
          ? FutureBuilder<List<Media>>(
              future:
                  provider.getPlaylistsMedia(playlists?.id ?? 0), //returns bool
              builder:
                  (BuildContext context, AsyncSnapshot<List<Media>> value) {
                if (value.data == null) {
                  return const Center();
                }

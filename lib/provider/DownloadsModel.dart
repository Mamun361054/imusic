...
import '../database/sqflite_database_provider.dart';
...
class DownloadsModel with ChangeNotifier {
  ...
  initDownloads(TargetPlatform platform, Downloads? downloads) async {
    ...
    await setLocalPath();
    if (downloads != null && isMediaInDownloads(downloads.id ?? 0) == null) {
      shouldRequestDownload = true;
      downloads.downloadUrl = downloads.streamUrl;
      String extension = p.extension(downloads.downloadUrl ?? '');
      downloads.streamUrl = "$localPath/${downloads.title?.replaceAll(' ', '')}$extension";
  ...
  void requestDownload(Downloads task) async {
    debugPrint("task.downloadUrl = ${task.downloadUrl}");
    String extension = p.extension(task.downloadUrl ?? '');
    ...
    task.taskId = await FlutterDownloader.enqueue(
        ...
        fileName: '${task.title?.replaceAll(' ', '')}$extension',
